...
> 像猫爪一样精准抓取服务器 - AI 增强的聊天式 SSH/RDP 运维桌面应用
...
```bash
# 1. 克隆仓库
git clone https://github.com/laikeylau/ops-claw-web.git
cd ops-claw-web
...
# 查看日志
docker compose logs -f ops-claw
```
...
services:
  ops-claw:
    ...
    environment:
      ...
      - SESSION_SECRET=change-me    # JWT 密钥
      - NODE_ENV=production
    volumes:
      - ops-claw-data:/app/data  # 数据持久化
```

**数据持久化：**

Docker 部署会自动创建 `ops-claw-data` 卷用于持久化数据：
- 服务器配置
- AI 配置
- 聊天记录
- 命令学习数据
...
# 或
PORT=3000 ADMIN_PASSWORD=your-password node out/server/index.js
```
...
# 启动应用
pm2 start out/server/index.js --name ops-claw-web
...
# 常用命令
pm2 status          # 查看状态
pm2 logs ops-claw-web  # 查看日志
pm2 restart ops-claw-web  # 重启
pm2 stop ops-claw-web     # 停止
```
...
| 提供商 | Endpoint | 模型示例 |
|--------|----------|----------|
| **小米 MiMo** | `https://token-plan-cn.xiaomimimo.com/v1` | MiMo-V2.5-Pro, MiMo-V2.5, MiMo-V2-Pro |
| OpenAI | `https://api.openai.com/v1` | gpt-4, gpt-4-turbo, gpt-3.5-turbo |
| Claude (Anthropic) | `https://api.anthropic.com/v1` | claude-3-opus, claude-3-sonnet, claude-3-haiku |
| DeepSeek | `https://api.deepseek.com/v1` | deepseek-chat, deepseek-coder |
| 智谱 AI (GLM) | `https://open.bigmodel.cn/api/paas/v4` | glm-4, glm-4-flash, glm-3-turbo |
| 百度文心 | `https://aip.baidubce.com/rpc/2.0/ai_custom/v1` | ernie-4.0, ernie-3.5, ernie-speed |
| 阿里通义 | `https://dashscope.aliyuncs.com/compatible-mode/v1` | qwen-turbo, qwen-plus, qwen-max |
| 月之暗面 (Kimi) | `https://api.moonshot.cn/v1` | moonshot-v1-8k, moonshot-v1-32k, moonshot-v1-128k |
| 本地 LLM (Ollama) | `http://localhost:11434/v1` | llama2, mistral, qwen2, yi |
