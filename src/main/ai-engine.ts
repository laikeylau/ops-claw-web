...
const SIMPLE_COMMAND_CACHE: Record<string, { command: string; explanation: string }> = {
  ...
  '列出容器': { command: 'docker ps -a', explanation: '查看所有 Docker 容器' },
  '容器列表': { command: 'docker ps -a', explanation: '查看所有 Docker 容器' },
  '查看docker镜像': { command: 'docker images', explanation: '查看所有 Docker 镜像' },
  '查看镜像': { command: 'docker images', explanation: '查看所有 Docker 镜像' },
  'docker镜像': { command: 'docker images', explanation: '查看所有 Docker 镜像' },
  'docker版本': { command: 'docker version', explanation: '查看 Docker 版本信息' },
  'docker信息': { command: 'docker info', explanation: '查看 Docker 系统信息' },
  '查看docker日志': { command: 'docker logs --tail 50 $(docker ps -q | head -1)', explanation: '查看最近的容器日志' },
  // 系统信息
  '查看磁盘': { command: 'df -h', explanation: '查看磁盘使用情况' },
  '磁盘空间': { command: 'df -h', explanation: '查看磁盘使用情况' },
  '磁盘使用': { command: 'df -h', explanation: '查看磁盘使用情况' },
  '查看内存': { command: 'free -h', explanation: '查看内存使用情况' },
  '内存使用': { command: 'free -h', explanation: '查看内存使用情况' },
  '内存占用': { command: 'free -h', explanation: '查看内存使用情况' },
  '查看cpu': { command: 'top -bn1 | head -20', explanation: '查看 CPU 使用情况' },
  'cpu使用': { command: 'top -bn1 | head -20', explanation: '查看 CPU 使用情况' },
  '系统信息': { command: 'uname -a', explanation: '查看系统信息' },
  '查看系统': { command: 'uname -a', explanation: '查看系统信息' },
  '系统负载': { command: 'uptime', explanation: '查看系统负载' },
  '查看进程': { command: 'ps aux | head -20', explanation: '查看运行中的进程' },
  '进程列表': { command: 'ps aux | head -20', explanation: '查看运行中的进程' },
  '当前目录': { command: 'pwd', explanation: '显示当前工作目录' },
  '查看目录': { command: 'ls -la', explanation: '查看当前目录文件列表' },
  '列出文件': { command: 'ls -la', explanation: '查看当前目录文件列表' },
  '文件列表': { command: 'ls -la', explanation: '查看当前目录文件列表' },
  // 网络
  '查看网络': { command: 'ip addr', explanation: '查看网络接口信息' },
  '网络接口': { command: 'ip addr', explanation: '查看网络接口信息' },
  '查看端口': { command: 'netstat -tlnp', explanation: '查看监听端口' },
  '监听端口': { command: 'netstat -tlnp', explanation: '查看监听端口' },
  '网络连接': { command: 'ss -tlnp', explanation: '查看网络连接' },
  // 服务
  '查看服务': { command: 'systemctl list-units --type=service --state=running', explanation: '查看运行中的服务' },
  '运行服务': { command: 'systemctl list-units --type=service --state=running', explanation: '查看运行中的服务' },
  // Git
  'git状态': { command: 'git status', explanation: '查看 Git 仓库状态' },
  'git日志': { command: 'git log --oneline -10', explanation: '查看最近 10 条 Git 提交记录' },
  'git分支': { command: 'git branch -a', explanation: '查看所有 Git 分支' },
  // 登录和日志
  '查看登录': { command: 'last -10', explanation: '查看最近登录用户' },
  '最近登录': { command: 'last -10', explanation: '查看最近登录用户' },
  '查看日志': { command: 'tail -50 /var/log/syslog', explanation: '查看系统日志' },
  '系统日志': { command: 'tail -50 /var/log/syslog', explanation: '查看系统日志' },
  // 时间和日期
  '当前时间': { command: 'date', explanation: '显示当前时间' },
  '查看时间': { command: 'date', explanation: '显示当前时间' },
  '系统运行时间': { command: 'uptime', explanation: '查看系统运行时间' },
};
 ...
 * 不同提供商可能需要不同的请求格式
 */
function detectProviderType(endpoint: string): 'openai' | 'anthropic' | 'baidu' | 'xiaomi' | 'custom' {
  const lower = endpoint.toLowerCase();
  if (lower.includes('anthropic') || lower.includes('claude')) {
    return 'anthropic';
  }
  if (lower.includes('baidu') || lower.includes('bce')) {
    return 'baidu';
  }
...
export class AIEngine {
  ...
  async generateCommand(
    ...
    try {
      ...
      const osHint = this.getOsHint(context.os || 'linux');
      const contextParts = this.buildContextParts(context);
      const contextInfo = contextParts.length > 0
        ? ` | 环境: ${contextParts.join(' | ')}`
        : '';

      // 精简提示词（减少约 50% token）
      const systemPrompt = `运维助手，将自然语言转为 shell 命令。只输出 JSON。${osHint}${contextInfo}
规则：只输出命令，安全第一，复杂操作用 &&。路径基于当前目录。
JSON: {"command":"命令","explanation":"简短解释"}`;
  ...
  async analyzeResult(
    ...
    try {
      const client = this.getClient(config);
            ...
            const osHint = context.os === 'windows' ? 'PowerShell' : 'bash';
      const contextParts = this.buildContextParts(context);
      const contextInfo = contextParts.length > 0
        ? ` | ${contextParts.join(' | ')}`
        : '';      // 精简分析提示词（减少约 60% token）
      const systemPrompt = `运维助手，分析命令结果并给建议。用${osHint}。${contextInfo}
JSON: {"analysis":"结果分析","suggestions":["建议"],"nextCommand":"后续命令(可选)","nextCommandReason":"原因(可选)"}`;
    ...
    try {
      ...
      const contextInfo = contextParts.length > 0
        ...
        : '';
            ...
            // 精简分解提示词（减少约 40% token）
      const systemPrompt = `运维助手，像经验丰富的工程师。简单直接，能一步完成的不用两步。

原则：简单任务1步，复杂任务最多${options.maxSteps}步，与服务器无关的对话返回空数组。
命令风格：直接用标准命令，不要复杂格式化。${osHint}${contextInfo}
可用工具：${options.allowedTools.join(',')}

JSON: {"subTasks":[{"id":"step1","description":"做什么","toolName":"ssh:execute","toolInput":{"command":"命令"}}],"reasoning":"思路"}`;
    ...
    try {
      ...
      if (historyContent.length > maxHistoryLength) {
        historyContent = historyContent.slice(-maxHistoryLength);
            ...
            // 精简摘要提示词
      const systemPrompt = `摘要助手，从操作历史提取关键信息。

要求：保留具体路径、重要数值（容器ID、端口、版本号），不要丢失上下文依赖信息。
JSON: {"summary":"摘要","keyFindings":["关键信息"],"successfulCommands":["成功命令"],"failedCommands":["失败命令"]}`;port OpenAI from 'openai';
import { AIConfigItem, SessionContext, CommandHistory } from './database';
...
请生成智能摘要。`;
  ...
  async *generateCommandStream(
    ...
    context: AIContext,
    config: AIConfigItem
  ): AsyncGenerator<{ chunk: string; fullContent: string }, AIGenerateResult> {
    const client = this.getClient(config);
...
JSON: {"command":"命令","explanation":"简短解释"}`;
    ...
    try {
      ...
      return {
        ...parsed,
        tokenUsage: undefined, // 流式模式下无法获取精确 token 使用
      };
    } catch {
      return {
        command: fullContent,
        explanation: 'AI 生成的命令',
      };
    }
  }
}
